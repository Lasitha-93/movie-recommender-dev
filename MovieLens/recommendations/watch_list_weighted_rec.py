...
new_user_profiles = pd.read_csv("../resource/u.user", delimiter="|", names=["user id", "age", "gender", "occupation", "zip code"])


def pearson_correlation_watchlist_weighted(person1_id, person2_id):
    ...
    # print(common_list)
    person1_rating_sum = 0
    person2_rating_sum = 0
    person1_square_rating_sum = 0
    person2_square_rating_sum = 0
    ...
    for item_id in common_list:
        ...
        person1_rating_sum += current_user1_rating
        person2_rating_sum += current_user2_rating

        person1_square_rating_sum += pow(current_user1_rating, 2)
        person2_square_rating_sum += pow(current_user2_rating, 2)

        product_sum_of_both_users += current_user1_rating*current_user2_rating

    # print(person1_rating_sum, person2_rating_sum, person1_square_rating_sum, person2_square_rating_sum, product_sum_of_both_users)
    # calculating pearson similarity:
    if len(common_list) != 0:
        numerator_value = product_sum_of_both_users - (
                    person1_rating_sum * person2_rating_sum / len(common_list))
        denominator_value = sqrt(
            (person1_square_rating_sum - pow(person1_rating_sum, 2) / len(common_list)) * (
                    person2_square_rating_sum - pow(person2_rating_sum, 2) / len(common_list)))
...
def weighted_user_similarity_by_watchlist(person_id):
    # similar_users = []
    scores = []
    for i, other_person in new_user_profiles.iterrows():
        ...
        weighted_sim_score = pearson_correlation_watchlist_weighted(person_id, other_person['user id'])
        scores.append([other_person['user id'], weighted_sim_score])
...
def create_weighted_watch_list_basic_sim_matrix():
    for k in range(1, 944, 1):
        u_all_sim = weighted_user_similarity_by_watchlist(k)
        u_only_sim = []
    ...
    np.savetxt("weighted_watchlist_basic_sim_matrix.csv", a, delimiter=",")

    df = pd.read_csv('weighted_watchlist_basic_sim_matrix.csv', sep=',', header=None)
    print(len(df))


# create_weighted_watch_list_basic_sim_matrix()
def make_recommendations(user_id):

    recommendation_list = {}
    watchlist_basic_sim_matrix = np.genfromtxt('weighted_watchlist_basic_sim_matrix.csv', delimiter=',')
    ...
    for unseen_movie in unseen_movies:
        ...
        for watched_user in watched_users:
            ...
            # print(current_other_user_sim)
            if current_other_user_sim > 0:
                if current_other_user_sim > available_highest_sim:
                    available_highest_sim = current_other_user_sim
                    available_best_match = watched_user
    #     get the rating from available_best_match
        if available_best_match > 0:
            rating = rating_data.loc[rating_data['user id'] == available_best_match]['rating'].values[0]
            ...
            recommendation_list[str(unseen_movie)] = str(rating)
...
def get_watch_list_weighted_rec():
    ...
    with open('watch_list_weighted_rec.json', 'w') as file:
        # indented_data=json.dumps(movies, indent=2)
        json.dump(all_recommendations, file)

    # with open('watch_list_weighted_rec.json') as file:
    #     new_data = json.load(file)
    #     indented_data=json.dumps(new_data, indent=2)
    #     print(indented_data)
    # final_list = np.asarray(all_recommendations)
    # np.savetxt("watch_list_weighted_rec.csv", final_list, delimiter=",")
    #
    # df = pd.read_csv('watch_list_weighted_rec.csv', sep=',', header=None)
    # print(len(df))


get_watch_list_weighted_rec()
