...
def feature_based_pearson_correlation(person1_id, person2_id):
...
def user_similarity_by_features(person_id):
    ...
    for i, other_person in new_user_profiles.iterrows():
        ...
        sim_score = feature_based_pearson_correlation(person_id, other_person['user id'])
...
def create_user_features_based_sim_matrix():
    for k in range(1, 944, 1):
        u_all_sim = user_similarity_by_features(k)
        u_only_sim = []
    ...
    df = pd.read_csv('user_features_based_sim_matrix.csv', sep=',', header=None)
    print('total rows: ', len(df))


# create_user_features_based_sim_matrix()
...
def get_user_feature_based_rec():
    ...
    with open('user_feature_based_rec.json', 'w') as file:
